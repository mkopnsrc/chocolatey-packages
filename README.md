...
- **[Static NuGet v3 feed on GitHub Pages](https://mkopnsrc.github.io/chocolatey-packages/)** — for embedded-binary packages (currently `fluent-bit`). One-step install by name with a Chocolatey source, no PAT required:
  ...
  Generated by [Sleet](https://github.com/emgarten/Sleet) on every `publish-nupkg-release.yml` run; retention capped at 5 stable versions per package. See [`docs/nuget-feed-design.md`](docs/nuget-feed-design.md).
