...
- **[GitHub Releases](https://github.com/mkopnsrc/chocolatey-packages/releases)** — the embedded-binary `.nupkg` is attached to a GitHub Release per version, tagged `<pkg>/<version>`. Works on any choco version (including choco 0.10.15) and doesn't require a NuGet source:

  ```powershell
  Invoke-WebRequest 'https://github.com/mkopnsrc/chocolatey-packages/releases/download/fluent-bit/5.0.8/fluent-bit.5.0.8.nupkg' -OutFile fluent-bit.nupkg
  choco install fluent-bit --source '.'
  ```

  Check the [Releases page](https://github.com/mkopnsrc/chocolatey-packages/releases) for the current version.
...
**From GitHub Releases** (embedded-binary packages like `fluent-bit`, works on any choco version):

```powershell
Invoke-WebRequest 'https://github.com/mkopnsrc/chocolatey-packages/releases/download/fluent-bit/5.0.8/fluent-bit.5.0.8.nupkg' `
...
| Package | Description |
|---|---|
| `fluent-bit` | Fluent Bit — CNCF Graduated telemetry agent for logs/metrics/traces (win64 MSI embedded). Releases tagged `fluent-bit/<version>`. See the [Releases page](https://github.com/mkopnsrc/chocolatey-packages/releases?q=fluent-bit) for available versions; **at most 5 versions kept at any time**. |
